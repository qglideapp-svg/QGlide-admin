...
export default function DeletePartnerModal({ isOpen, onClose, onConfirm, email, displayName, isLoading }) {
  const { t } = useLanguage();
    ...
    <div className="modal-overlay" role="presentation" onClick={handleClose}>
      <div className="modal-content delete-modal-content" role="dialog" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2>{t('partners.deleteModalTitle')}</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <div className="danger-banner">
              ...
              <p>
                <strong>{t('modals.danger')}:</strong> {t('partners.deleteModalWarning')}
              </p>
            </div>
            <p>
              {t('partners.deleteModalBody')} <strong>{label}</strong>
            </p>
            <div className="form-group">
              <label htmlFor="deletePartnerReason">{t('modals.reasonForDeletion')}</label>
              <textarea
                id="deletePartnerReason"
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder={t('partners.deleteReasonPlaceholder')}
                required
                rows={4}
                disabled={isLoading}
              />
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn-delete" disabled={isLoading || !reason.trim()}>
              {isLoading ? t('partners.deleting') : t('partners.delete')}
            </button>
          </div>
        </form>
      </div>
